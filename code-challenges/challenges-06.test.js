...
const sortByChildren = (charArray) => {
  // Solution code here...
  return charArray.sort((a,b) => a.children.length > b.children.length ? 1 : a.children.length < b.children.length ? -1: a.house > b.house ? 1 : -1)
};
/* ------------------------------------------------------------------------------------------------
CHALLENGE 2
...
const getHouses = (arr) => {
  let houses = [];
  // Solution code here...
  for(let key in arr)houses.push(arr[key].house)
  return houses;
};
...
const totalCharacters = (arr) => {
  // Solution code here...
  let count = 0;
  for (let i = 0 ; i < arr.length ; i++)arr[i].spouse? count = count + 2 + arr[i].children.length: count = count + 1 + arr[i].children.length ;
  return count; 
};
...
const houseSize = (arr) => {
  ...
  // Solution code here...
  arr.forEach(ele => {
    ele.spouse?sizes.push({house: ele.house, members:(2 + ele.children.length)}):sizes.push({house: ele.house, members:(1 + ele.children.length)}) ;
  });
  return sizes;
};
...
const houseSurvivors = (arr) => {
  ...
  // Solution code here...
  arr.forEach(ele => {
    deceasedSpouses.includes(ele.spouse) == false  && ele.spouse ? survivors.push({house: ele.house, members:(2 + ele.children.length)}) : survivors.push({house: ele.house, members:(1 + ele.children.length)}) ;
  });
  return survivors;
};
